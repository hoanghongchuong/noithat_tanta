	...
	<section class="bread-wrap ic-detail-bread">
		<div class="container">
			...
			<ul class="s15 list-unstyled justify-content-center bread op7 text-center">
				...
				<li><a href="inter-child.html" title="">Thiết kế thi công @if($project->com == 'noi-that') nội thất @else ngoại thất @endif</a></li>
				<li>{{$project->name}}</li>
			</ul>
		</div>
	</section>
					...
					<div class="col-lg-105 col-md-4 col-sm-6">
						<div class="pj-item">
							<figure class="text-center pj-item-img">
								<a href="{{ url('thiet-ke-thi-cong-'.$p->com.'/'.$p->alias.'.html') }}" title="{{$p->name}}"><img src="{{asset('upload/news/'.$p->photo)}}" title="{{$p->name}}" alt="{{$p->name}}"></a>
							</figure>
							
							<figcaption class="pj-item-info">
								<h3 class="text-center"><a href="{{ url('thiet-ke-thi-cong-'.$p->com.'/'.$p->alias.'.html') }}" title="{{$p->name}}">{{$p->name}}</a></h3>
							</figcaption>
						</div>
					</div>
					@endforeach
